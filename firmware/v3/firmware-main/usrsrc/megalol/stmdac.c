...
*******************************************************************************/
void stmdac_init(unsigned int samplerate)
{
	HAL_StatusTypeDef s;
	...
	if(samplerate!=0)
	{
		...
		unsigned pclk = (unsigned)HAL_RCC_GetPCLK1Freq();
...
/******************************************************************************
	function: _stm_dac_clearwaveforms
*******************************************************************************
	Clears all waveforms and clears the DAC buffer.
...
void _stm_dac_clearwaveforms()
{
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
	{
		for(unsigned i=0;i<DACDMAHALFBUFSIZ*2;i++)
			_stm_dac_dmabuf[i]=0;
...
void _stm_dac_initwaveforms(unsigned n,...)
{
	...
	for(unsigned i=0;i<n;i++)
	{
		...
		int rv = stm_dac_addwaveform(f,4095);
...
void HAL_DAC_ConvCpltCallbackCh1(DAC_HandleTypeDef *hdac)
{
	...
	for(unsigned i=0;i<DACDMAHALFBUFSIZ;i++)
	{
		_stm_dac_dmabuf[DACDMAHALFBUFSIZ+i] = 0;
		for(int w=0;w<_stm_dac_numwaveforms;w++)
		{
			...
			_stm_dac_dmabuf[DACDMAHALFBUFSIZ+i] += s;
...
void HAL_DAC_ConvHalfCpltCallbackCh1(DAC_HandleTypeDef *hdac)
{
	...
	for(unsigned i=0;i<DACDMAHALFBUFSIZ;i++)
	{
		...
		//_stm_dac_dmabuf[i] /= _stm_dac_numwaveforms;			// Rescale to ensure same amplitude regardless of number of waveforms
		//_stm_dac_dmabuf[i] /= _stm_dac_ampldiv;
	}
...
void stmdac_benchmaxspeed()
{
	...
	{
		stm_dac_addwaveform(20000-nw*1000,4096);			// Add waveform
...
void stmdac_benchcpuoverhead()
{
	...
	for(unsigned c=0;1;)
	{
		...
		for(int i=0;i<nw;i++)
		{
			stm_dac_addwaveform(sr_frq[c+i],4096);
