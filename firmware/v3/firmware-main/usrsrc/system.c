...
{
	// Turn on LDO
	HAL_GPIO_WritePin(PWR_MOTION_EN_GPIO_Port, PWR_MOTION_EN_Pin, GPIO_PIN_SET);
	//HAL_Delay(200);	// Wait power ok
	HAL_Delay(20);	// Wait power ok
#warning Must enable SPI at this stage
}
void system_motionvcc_disable()
{
	...
	HAL_Delay(20);	// Wait power ok
#warning Must disable (tristate) SPI at this stage
}
