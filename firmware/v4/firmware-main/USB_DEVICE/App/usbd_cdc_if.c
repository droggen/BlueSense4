...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	for(int i=0;i<*Len;i++)
	{
		if(!buffer_isfull(&SERIALPARAM_USB.rxbuf))
		{
			buffer_put(&SERIALPARAM_USB.rxbuf,Buf[i]);
		}
		else
		{
			// If buffer is full we lose the most recent data
			break;
		}
	}


	// This is for the next data reception
