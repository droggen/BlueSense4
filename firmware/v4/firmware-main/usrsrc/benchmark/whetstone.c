...
unsigned whetstone(int continuous,int loopstart,FILE *file)
{
	...
	// Replace command line parsing by parameters to function
...
*/
	...
	if (finisec-startsec <= 1000) {
		fprintf(file,"Insufficient duration- Increase the LOOP count\n");
		return(1);
	}

	fprintf(file,"Loops: %ld, Iterations: %d, Duration: %ld ms.\n",
			LOOP, II, finisec-startsec);

	KIPS = (100.0*LOOP*II)*1000.0/(float)(finisec-startsec);
	if (KIPS >= 1000.0)
		fprintf(file,"C Converted Double Precision Whetstones: %.1f MIPS\n", KIPS/1000.0);
	else
		fprintf(file,"C Converted Double Precision Whetstones: %.1f KIPS\n", KIPS);
...
{
	fprintf(file,"%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e\n",
						N, J, K, X1, X2, X3, X4);
}
#endif
